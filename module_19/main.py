...
from yolov5.utils.dataloaders import orientation
...
fig=px.pie(filtered_books_df,names='Genre',title='Most Liked Genre(2009-2002', color='Genre',color_discrete_sequence=px.colors.sequential.Plasma)
st.plotly_chart(fig)
...
fig = px.bar(size,x='Year',y='Counts',color='Genre',title='Number of Fiction vs Non-Fiction Books from 2009-2022',color_discrete_sequence=px.colors.sequential.Plasma,barmode='group')
...
fig = px.bar(top_authors, x='Count', y='Author', orientation='h', title="Top 15 Authors by Counts of Books Published", labels={'Count': 'Counts of Books Published', 'Author': 'Author'}, color='Counts', color_continuous_scale=px.colors.sequential.Plasma)
st.plotly_chart(fig)

st.header("Filter Data by Genre")
