class Vertebrate:
    def __init__(self, backbone=True):
        self.has_backbone = backbone

    def vertebrate_info(self):
        print("Vertebrates have a backbone.")

class Aquatic:
    def __init__(self, habitiat = "water"):
        self,habitiat = habitiat

    def aquatic_info(self):
        print("Aquatic animals live in water.")

class Fish(Vertebrate, Aquatic):
    def __init__(self, species, backbone = True, habitat = "water"):
        super().__init__(backbone = True)
        self.habitat = habitat
        self.species = species

    def fish_info(self):
        print(f"The {self.species} is a type of fish found in {self.habitat}")

    def swim(self):
        print("The fish is swimming")

Blerdonifish = Fish("Blerdonifish")
print(Blerdonifish.has_backbone)
print(Blerdonifish.habitat)
Blerdonifish.vertebrate_info()
Blerdonifish.aquatic_info()
Blerdonifish.fish_info()
Blerdonifish.swim()